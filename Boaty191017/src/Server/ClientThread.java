...
class ClientThread implements Runnable {
     ...
     */
    public ClientThread(Socket clientSocket, String version, Object camera) {
        try {
    ...
    private String doCommandList(int fromClient) {
        ...
        switch (fromClient) {
            case MsgType.START_VIDEO_STREAM:
                new Thread(new VideoStream(camera)).start();
                msg = "Videostream started";
                break;

            case MsgType.MANUAL:
                Server.addServerData("Man/Auto", 0);
                break;
            case MsgType.AUTO:
                Server.addServerData("Man/Auto", 1);
                break;
        }
        return msg;
    }
